...
\institute{E.A. Roualdes \and S. J. Bonner
...
A more recent method, by \citet{Agusti:2003}, esimates predator's preferences with unobserved count data using a Monte Carlo approach, the bootstrap to be more accurate.  This method deserves credit for its attempt to do away with distributional assumptions, however we find that the lack of a distribution causes some unresolved problems.  \citet{Agusti:2003} suggest resampling the data to estimate proprotions of prey species present in the mesocosms\footnote{Mesocosm was defined by \citet{Agusti:2003} and is there assumed representative of the predator's habitat.} and in predators' guts.  A seemingly intuitive null hypothesis to test random feeding states that pairs of proportions, relative to a given prey species, are equal.  For instance, if the proportion of prey species $j$ in the mesocosm is denoted $\pi_j$, and $p_j$ denotes the proportion of prey species $j$ in the predator's gut, then their null hypothesis is written $\pi_j = p_j, \, \forall j$.  Since the $\pi_j$s depend on the unknown number of prey species consumed, this null hypothesis is not easily tested.  Even if one assumed known the distribution of prey species consumed, this assumed distribution would depend on unkown parameters, thus only complicating any derived null hypothesis further.  Exactly what \citet{Agusti:2003} are testing in their paper is unclear.

The model presented here is a formal statistical model, which enables clear and intuitive hypothesis testing of multiple prey species across an array of time points.  Our methods, via a likelihood ratio test, offer statistically powerful conclusions about the hypotheses of interest.  Robust simulations provided insight into the statistical properties of our model.  Moreover, we can estimate parameters of interest even when the predator's gut contents are not fully observed.  

Throughout this paper we assume the observed data were generated from underlying Poisson distributions.  Though this assumption is not easily tested, and other factors affecting our model may distort our methods, e.g.\ sampling issues, environmental variables, differential decay rates, we do not believe a better assumption is readily available to handle all the cases we considered.  The Poisson distribution provides reasonable biological interpretation to the available data, data obtained by ecologists interested in prey electivity.  One could hypothesize a more complex distribution that similarly has good biological interpretation, say a distribution with more than one parameter.  This may perform better than our model when full count data are observed.  But when only unobserved count data is available, any distribution with more than one parameter is unidentifiable.  Further, because the Poisson distribution {\color{red} SAY SOMETHIGN}, the EM algorithm is theoretically guaranteed to converge to the parameters of interest {\color{red} CITATION? Hoeting and Givens}.  Future development of our methods might consider more complex distributions to handle variables not presently considered in the case of fully observed count data.

{\color{red} So what does our model offer ecologists? Multiple prey species and time points, contrasts, unobserved count data.  Just a quick paragraph. It should also mention our package.}
