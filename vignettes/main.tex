...
Unbalanced data isn't actually ready yet.
...
To see that $c$ is estimated accurately, we use the fact that $\gamma_{st} = \gamma$ for all $\{s,t\}$ by averaging over the $4$ values of $\boldsymbol{\gamma}$ that were sampled: $\bar{\hat{c}}*\bar{\hat{\gamma}} = 1.4956$. 
...
\hlstd{l} \hlkwb{<-} \hlkwd{matrix}\hlstd{(}\hlkwd{sqrt}\hlstd{(}\hlnum{2}\hlstd{),} \hlkwc{nrow} \hlstd{= Times,} \hlkwc{ncol} \hlstd{= PreySpecies)}
\hlstd{g} \hlkwb{<-} \hlkwd{matrix}\hlstd{(}\hlnum{5}\hlstd{,} \hlkwc{nrow} \hlstd{= Times,} \hlkwc{ncol} \hlstd{= PreySpecies)}
\hlkwd{system.time}\hlstd{(out} \hlkwb{<-} \hlkwd{testPref}\hlstd{(PreySpecies, Times, Predators,}
...
The same as before, $c$ is estimated fairly well: $\bar{\hat{c}}*\bar{\hat{\gamma}} = 1.4188$.  Also, we can check how many iterations of the EM algorithm the last simulated dataset required
